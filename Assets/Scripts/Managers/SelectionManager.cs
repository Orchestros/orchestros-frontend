...
namespace Managers
{
    ...
    public class SelectionManager : MonoBehaviourWithState
    {
        ...
        ///     <para>
        ///         If the left button is clicked, sends a raycast and retrieves the objet it collided with. If the object
        ///         possesses the
        ///         tag SelectableObject, selects it. Otherwise, the panel selection process allowing to select multiple objects at
        ///         the same time is launched.
        ///         Essentially, this means that the _isCurrentlySelecting is set to true. At the same time the _startPointCanvas
        ///         and _startHitPoint are saved for later use by the UpdateSelection
        ...
        {
            foreach (var selectedObject in _selectedObjects.ToList()) ToggleObjectSelection(selectedObject);
